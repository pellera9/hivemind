...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join20(homedir13(), ".claude", "settings.json");
...
function ensurePluginNodeModulesLink(opts) {
  ...
  const target = opts.sharedNodeModules ?? join21(homedir14(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname8(opts.bundleDir);
  const link = join21(pluginDir, "node_modules");

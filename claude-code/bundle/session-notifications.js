...
async function fetchOpenGoals(creds, goalsTableName) {
  ...
  try {
    ...
    const sql = (
      ...
      `SELECT goal_id, owner, status, content FROM "${safe}" WHERE owner LIKE '%${sqlStr(creds.userName)}%'   AND status IN ('opened', 'in_progress') ORDER BY created_at DESC LIMIT 25`
    );
    ...
    for (const r of rows) {
      ...
      if (owner !== u && !owner.includes(u) && !u.includes(owner))
        continue;

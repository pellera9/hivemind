...
async function executeBackfill(toExtract, opts) {
  ...
  const summary = { attempted: 0, staged: 0, embedded: 0, failed: 0, timedOutOnBudget: false };
...
function releaseBackfillLock() {
  if (process.env.HIVEMIND_BACKFILL_LOCK_OWNED !== "1")
    return;
  try {
    if (existsSync40(PENDING_MEMORY_LOCK_PATH))
      unlinkSync15(PENDING_MEMORY_LOCK_PATH);
  } catch {
  }
}
...
async function runExtract(plan, cwd) {
  ...
  process.stdout.write(`staged ${result.staged}/${result.attempted} session(s) (${result.embedded} embedded, ${result.failed} failed${result.timedOutOnBudget ? ", budget reached" : ""}). Sign in and run the flush to push them into team memory.
`);
  void homedir31;
...
function findHivemindLauncher() {
  ...
  try {
    const out = execFileSync7("which", ["hivemind"], {
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "ignore"]
    });
    const bin = out.trim();
